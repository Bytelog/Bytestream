...
#define CREATE_LOG_PROTOTYPE(level_, ...)                                      \
    void log_##level_(                                                         \
        const char *fmt,                                                       \
        ...                                                                    \
    ) __attribute__((format(printf, 1, 2))) __VA_ARGS__;

#define log_debug(fmt, ...)

#else

// For debug configuration, also include File, Function, and Line number
#define CREATE_LOG_PROTOTYPE(level_, ...)                                      \
    void log_##level_##_debug(                                                 \
        const char *file,                                                      \
        const char *fun,                                                       \
        int line,                                                              \
        const char *fmt,                                                       \
        ...                                                                    \
    ) __attribute__((format(printf, 4, 5))) __VA_ARGS__;
