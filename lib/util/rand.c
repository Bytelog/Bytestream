...
struct rand_ctx* rand_new() {
    struct rand_ctx *ctx = calloc(1, sizeof(*ctx));

    FILE* urandom = fopen("/dev/urandom", "r");
    if (UNLIKELY(urandom == NULL)) {
        goto error;
    }

    uptr result = fread(ctx->mem, sizeof(ctx->mem[0]), RANDSIZ, urandom);
    if (UNLIKELY(result < RANDSIZ)) {
        goto error;
    }
...
f64 rand_f64(struct rand_ctx* ctx) {
    // Divide into buckets the size of the effective mantissa
    return (_rand(ctx) >> 11) * (1.0 / 9007199254740992.0);
}

f32 rand_f32(struct rand_ctx* ctx) {
    // Divide into buckets the size of the effective mantissa
    return (_rand(ctx) >> 8) * (1.0 / 8388608.0);
}

i32 rand_range(struct rand_ctx* ctx, i32 m, i32 n) {
    return m + _rand(ctx) / ((u64) -1 / (n - m + 1) + 1);
}

f64 rand_normal(struct rand_ctx* ctx, f64 mean, f64 stddev) {
    return rand_standard(ctx) * stddev + mean;
}

/**
 * The Ziggurat Method
 *
 * Implementation ported from The Go Authors, and is BSD licensed.
 * https://golang.org/src/math/rand/normal.go
 */

const f64 rn = 3.442619855899;
...
f64 rand_standard(struct rand_ctx* ctx) {
    ...
    for (;;) {
        ...
        if (LIKELY(abs32(j) < kn[i])) {
            return x;
        }

        if (i == 0) {
            while (true) {
                x = -log(rand_f64(ctx) * (1.0 / rn));
                y = -log(rand_f64(ctx));
                if (y + y > x * x) {
                    break;
                }
        ...
        if (fn[i] + rand_f32(ctx) * (fn[i-1] - fn[i]) < (f32) exp(-0.5*x*x)) {
            return x;
        }
    }
}
